...
# ========================
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    users = User.joins(:orders, :order_items).where(order_items: {item_id: @item_8.id}).order(:name).distinct.pluck(:name)
    # ------------------------------------------------------------
  ...
  it '17. returns the name of items associated with a specific order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    names = Item.joins(:orders).where(orders: {id: @order_12.id}).order(id: :asc).pluck(:name)
    # ------------------------------------------------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    items_for_user_3_third_order = Item.joins(:orders).where(orders: {id: @user_3.orders[2].id}).order(id: :asc).pluck(:name)
    # ------------------------------------------------------------
...
# ========================
  ...
  it '24. returns all orders for user 2 which include item_4' do
    ...
    # ------------------ Improved Solution ----------------------
    #  Solution goes here
    orders = Order.joins(:items).where(user: @user_2, items: {id: @item_4.id})
    # -----------------------------------------------------------
  ...
  it '25. returns items that are associated with one or more orders' do
    ...
    # Solution goes here
    ordered_items = Item.joins(:orders).group(:id).order(id: :asc)
    # ---------------------------------------------------------------
...
# ========================
  ...
  it '26. returns the names of items that are associated with one or more orders' do
    ...
    ordered_items_names = Item.joins(:orders).group(:id).order(id: :asc).pluck(:name)
    # ---------------------------------------------------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
     custom_results = User.joins(:orders).select("users.name, count(orders.user_id) as total_order_count").group(:name)
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
     custom_results = User.joins(orders: :items).select("users.name, count(items.id) as total_item_count").group(:name).order(name: :desc)
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
      data = User.joins(orders: :order_items).select("users.name as user_name, orders.id as order_id, orders.amount / count(order_items.id) as avg_item_cost")
     .group("users.name, orders.id")
     .order("users.name desc, orders.id asc")
    # ---------------------------------------------------------------
  ...
  it '30. returns the names of items that have been ordered without n+1 queries' do
    ...
    # ------------------------------------------------------
    orders = Order.all.includes(:order_items, :items) # Edit only this line
    # ------------------------------------------------------
